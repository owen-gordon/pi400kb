...
# Using ECM with pi500usb

If you are combining these steps with the `pi500usb` gadget so that you can share a keyboard/mouse and Ethernet over the same USB-C connection:

1. Complete the configuration above to create an `ethernet-usb0` NetworkManager connection.
2. Build `pi500usb` with the combined HID+ECM gadget support (defaults expect `-DECM_NM_CONNECTION=ethernet-usb0`).
3. When you start `pi500usb`, it will automatically bring up the connection so your host gets an IP over USB without manual `nmcli` commands.
4. Stopping `pi500usb` will drop the connection again, restoring your local keyboard.
